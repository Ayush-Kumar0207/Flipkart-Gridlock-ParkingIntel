...
def compute_impact_scores(df, output_dir='dashboard/data'):
    """
    Compute CIS = α×density + β×main_road_frac + γ×heavy_vehicle_ratio + δ×peak_hour_concentration
    for each junction and zone.
    """
    ...
    # Compute CIS (weights: density=0.4, main_road=0.25, heavy_vehicle=0.2, peak=0.15)
    ...
    junction_stats['norm_density'] = normalize(junction_stats['count'])
    ...
    junction_stats['cis'] = (
        ...
        0.25 * junction_stats['norm_main_road'] +
        0.20 * junction_stats['norm_heavy'] +
        0.15 * junction_stats['norm_peak']
    ) * 100  # Scale to 0-100
    ...
    station_stats['norm_density'] = normalize(station_stats['count'])
    station_stats['cis'] = (
        0.40 * station_stats['norm_density'] +
        0.25 * station_stats['main_road_frac'] +
        0.20 * station_stats['heavy_vehicle_ratio'] +
        0.15 * station_stats['peak_hour_concentration']
    ) * 100
    ...
    impact_data = {
        ...
        'formula': 'CIS = 0.40×density + 0.25×main_road_frac + 0.20×heavy_vehicle + 0.15×peak_concentration',
    }
